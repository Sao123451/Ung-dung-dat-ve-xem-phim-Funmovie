...
//  - mode=grid: nhóm theo row, sort number tăng dần: [{row:"A", seats:[{_id,number,seat_type,extra_price}, ...]}, ...]
//  - mode=list: trả list phẳng (mặc định)
exports.publicByRoom = async (req, res, next) => {
  try {
    ...
    const seats = await Seat.find({ room })
      .select('_id row number seat_type extra_price')
      .sort({ row: 1, number: 1 })
      .lean();
    ...
    if (mode === 'grid') {
      ...
      for (const s of seats) {
        ...
        map.get(s.row).push({ _id: s._id, number: s.number, seat_type: s.seat_type, extra_price: s.extra_price });
      }
...
// GET /api/seats?room=<roomId>&row=A&type=vip&page=1&limit=100
exports.list = async (req, res, next) => {
  try {
    const { room, row, type, page = 1, limit = 100 } = req.query;
    const filter = {};
    if (room) {
      if (!isId(room)) return res.status(400).json({ message: 'Invalid room id' });
      filter.room = room;
    ...
    const [items, total] = await Promise.all([
      Seat.find(filter).sort({ row: 1, number: 1 }).skip(skip).limit(parseInt(limit,10)),
...
// POST /api/seats  body: { room, row, number, seat_type?, extra_price? }
exports.create = async (req, res, next) => {
  try {
    const { room, row, number, seat_type, extra_price } = req.body;
...
// POST /api/seats/bulk
// body: { room, rows: [ { row:"A", from:1, to:10, seat_type?,"extra_price"? }, ... ] }
// tạo dãy ghế nhanh theo từng hàng
exports.bulkCreate = async (req, res, next) => {
  try {
    ...
    // ordered:false => duplicate keys will throw BulkWriteError; vẫn coi là thành công một phần
...
// DELETE /api/seats/clear?room=<roomId>  (xóa toàn bộ ghế của 1 phòng)
